...
|   3.1   | [**Technical Considerations**](#Chapter-3-1)      |  5   |
|   3.2   | [**Part 1 - Libc functions**](#Chapter-3-2)       |  6   |
|   3.3   | [**Part 2 - Additional functions**](#Chapter-3-3) |  7   |
|    4    | [**Bonus part**](#Chapter-4)                      |  11  |
...
##### _C programming can be very tedious when one doesn’t have access to those highly useful standard functions. This project is about understanding the way these functions work, implementing and learning to use them. You will create your own library. It will be helpful since use it in your next C school assignments._

C 프로그래밍을 할 때 매우 유용한 표준 함수들을 사용할 수 없다면 굉장히 지루할 겁니다. 이 프로젝트를  통해 이러한 표준 함수들이 어떻게 작동하는지 이해하고, 구현하고 어떻게 사용하는 지 배울 수 있습니다. 당신만의 라이브러리를 만들 것입니다. 이 라이브러리는 나중에 진행할 C 프로젝트에 도움이 될 것입니다.

##### _Take the time to expand your `libft` throughout the year. However, when working on a new project, don't forget to ensure the functions used in your library are allowed in the project guidelines._

과제를 진행하면서 당신만의 libft를 확장시켜 보세요. 그러나 새로운 프로젝트를 진행할 때는 라이브러리에 사용된 함수가 프로젝트 가이드라인에서 허용되는지 확인하는 것을 잊지 마세요.
...
- ##### _Declaring global variables is forbidden._
...
- ##### _Place all your files at the root of your repository._
  ...
  모든 파일을 레포지토리의 루트에 제출하세요.

- ##### _Turning in unused files is forbidden._

  사용하지 않는 파일의 제출은 금지됩니다.

- ##### _Every .c must compile with flags `-Wall -Wextra -Werror`_

  모든 .c 파일은 `-Wall -Wextra -Werror` 플래그와 함께 컴파일되어야 합니다.
...
- ##### _Your libft.a has to be created at the root of your repository._

  libft.a는 반드시 레포지토리의 루트에 생성되어야 합니다.
